...
import {
  DbTvshow, Episode, UserEpisodeStatus, UserTvshowStatus,
} from '../../api/src/types';
...
export default class TvShows extends React.Component<TvShowCardProps, TvShowCardState> {
  ...
  handleClick(evt: React.MouseEvent<HTMLElement>): void {
    ...
    ctx.router.navigateTo(`#/tvshow/${tvshow.tmdbid}`);
  }

  handlePlayTvshow(evt: React.MouseEvent<HTMLElement>): void {
    const { tvshow } = this.props;
    const episode: Episode | undefined = playTvshow(tvshow, undefined);
    if (episode) {
      const us: UserEpisodeStatus | null = getEpisodeUserStatus(episode);
      this.setState({
  ...
  render(): JSX.Element {
    const { tvshow, showNext } = this.props;
    const userStatus = getTvshowUserStatus(tvshow);
    const infos = [];
    let progress: JSX.Element | null = null;
    let posterPath: string = '';
    if (showNext) {
      const episode: Episode | undefined = selectCurrentEpisode(tvshow);
      if (episode) {
        infos.push(
          <span key={1} className="next-episode text-center">
    ...
    return /* <React.Fragment> */(
      ...
      >
        <span className="poster">
          { posterPath
            ? <img src={`/images/posters_w342${posterPath}`} alt={tvshow.title} width="342" height="513" loading="lazy" />
            : (
          ...
          <b onClick={this.handlePlayTvshow.bind(this)} className={ctx.hasHover ? '' : 'd-none'}>
          ...
          <i className={ctx.hasHover ? '' : 'd-none'}>
            <em
              title="Pas intéressé"
